...
#if defined(NDEBUG)
# error "Fifacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1e8dc8d21fb40e5f52e396bb7d727d1599cec32670c3c61c085a95d66fa30589");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Fifacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Fifacoin Signed Message:\n";
...
{
    // Fifacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Fifacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Fifacoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Fifacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Fifacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Fifacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb74e0486a22ce1c642e316becaefbe26efcdd44b88ddd777e1ffee473a6067c9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "COVID-19 A VIRUS THAT MADE MANY FAMILIES CRY...START 2020!!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04307945110dadfdfe7230eb0ddb5863da83b243b9e8af431f2fa93ae24ecc6f0aa533e0ca03849471ec2b92214a5cc5ff10db67a9793eddad5e872540704680e1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1621396606;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085051872;

        if (fTestNet)
        {
            block.nTime    = 1621396536;
            block.nNonce   = 385309254;
        }
...
// If genesis block hash does not match, then generate new genesis hash.
        ...
        assert(block.hashMerkleRoot == uint256("0x54a26b6bd097d099dc20add86b670e7974aa2e352474b8c64f275d7bcdfaefcc"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xb1, 0xdc }; // Fifacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FifacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FifacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FifacoinMiner(CWallet *pwallet)
{
    printf("FifacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fifacoin-miner");
    ...
    try { loop {
        ...
        printf("Running FifacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FifacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FifacoinMiner, pwallet));
}
